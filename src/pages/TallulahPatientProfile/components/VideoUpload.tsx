...
import { TMediaFileUpload } from './EditPatientProfileModal/EditPatientProfileModal';
...
const VideoUpload: React.FC<IUploadMedia> = ({ fieldName, setVideoFiles }) => {
  ...
  useEffect(() => {
    if (setVideoFiles) {
      setVideoFiles((prev: TMediaFileUpload[]) => {
        const newDocumentFiles: TMediaFileUpload[] = [...prev];
